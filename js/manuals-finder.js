 ...
 * @date 1/7/2015
 *
 * Same as parts-finder with modified queries for manuals
 */
...
(function(root, $, document, undefined){
  ...
  function manualsFinderinit() {
    // set up our scaffold
    $('[data-manuals-finder]').each(function() {
  ...
  function getBrands(el, cat) {
    var q = JSON.stringify({
          Attributes: {
    		    $elemMatch: {
    	        $and: [{ID: 'Manual'}, {Name: {$regex: '(manual|Manual)'}}]
    		    }
    			},
          Displayable: 1,
          Categories: cat
      ...
      success : function(data) {
        ...
        .on('change', function(e){
          ...
          if($('.no-touch').length) { // attempt to keep the new windows only to desktop
            var win = window.open('http://www.horizonhobby.com/' + this.value + '?clickpath='+$(this).closest('.parts-bar').data('cat')+'_partsfinder#support');
	            if(win === undefined) { // popup blocker on eg. safari has it on by default and does not even tell the user that it blocked the new window so the user thinks your script is broken,
		            location.href = 'http://www.horizonhobby.com/' + this.value + '?clickpath='+$(this).closest('.parts-bar').data('cat')+'_partsfinder#support';
	            }
          } else {
            location.href = 'http://www.horizonhobby.com/' + this.value + '?clickpath='+$(this).closest('.parts-bar').data('cat')+'_partsfinder#support';
          }
